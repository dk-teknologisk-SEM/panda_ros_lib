...
class IterativeParabolicTimeParameterization:
    ...
    def compute_time_stamps(self, plan: RobotTrajectory, speed):
        ...
        #         distance += (plan.joint_trajectory.points[i].positions[j] - plan.joint_trajectory.points[i-1].positions[j])**2
        #     distance = distance**0.5
        #     time_stamps.append(time_stamps[i-1] + distance/speed)
        # return time_stamps
            ...
            for idx, v in enumerate(vars):
                for count in range(2):
                    for i in range(num_points-1):
                        ...
                        if abs(a) > a_max + 0.1:
                            if not backwards:
                                dt2 = min(dt2 + 0.01, self.findT2(q2-q1,q3-q2,dt1,dt2,a_max))
                                time_diff[index] = dt2
                            else:
                                dt1=min(dt1+0.01, self.findT1(q2-q1,q3-q2,dt1,dt2,a_max))
    ...
    # def apply_time_parameterization(self, plan, time_stamps):
    #     print(time_stamps)
    #     print(len(plan.joint_trajectory.points))        
    #     for i in range(1, len(plan.joint_trajectory.points)):
    #         plan.joint_trajectory.points[i].time_from_start = rospy.Duration(time_stamps[i])
    #     return plan
    ...
    def update_trajectory(self, plan:RobotTrajectory, time_diff):
        ...
        for i in range(1, num_points):
            ...
            plan.joint_trajectory.points[i].time_from_start = rospy.Duration(time_sum)
